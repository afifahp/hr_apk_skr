...
class LeaveService {
  ...
  /// Ambil header auth pakai api_key + api_secret
  Future<Map<String, String>> _getHeaders({bool isMultipart = false}) async {
    final prefs = await SharedPreferences.getInstance();
  ...
  Future<List<LeaveRequest>> fetchLeaveRequests(
    ...
    final headers = await _getHeaders();
    final url = Uri.parse("http://127.0.0.1:8000/api/method/hrpay.api.leave.get_all_leave_requests");
  ...
  Future<List<LeaveRequest>> fetchLeaveRequestsAuth(
    ...
    final headers = await _getHeaders();
    final url = Uri.parse("http://127.0.0.1:8000/api/method/hrpay.api.leave.get_leave_request_by_auth?user_id=$userId");
  ...
  Future<bool> createLeaveRequest(
    ...
    final url = Uri.parse("http://127.0.0.1:8000/api/method/hrpay.api.leave.create_leave_request");
    final headers = await _getHeaders();
  ...
  Future<bool> updateLeaveStatus(
    ...
    final headers = await _getHeaders();
    final url = Uri.parse("http://127.0.0.1:8000/api/method/hrpay.api.leave.update_leave_status?leave_request_id=$requestId");
  ...
  /// Ambil detail cuti berdasarkan Department
    Future<List<LeaveRequest>> fetchLeaveByDepartment(String department) async {
    final headers = await _getHeaders();
    final url = Uri.parse(
      "http://127.0.0.1:8000/api/method/hrpay.api.leave.get_leave_by_department?department=$department",
    );

    final res = await http.get(url, headers: headers);
    if (res.statusCode != 200) {
      throw Exception(
        "Gagal ambil data cuti (department): ${res.statusCode} ${res.body}",
      );
    }
  ...
  Future<LeaveRequest> fetchLeaveDetail(String id) async {
    final headers = await _getHeaders();
    final url = Uri.parse("http://127.0.0.1:8000/api/method/hrpay.api.leave.get_leave_request_detail?leave_request_id=$id");
